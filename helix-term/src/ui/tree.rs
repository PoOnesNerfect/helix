...
impl<T: TreeViewItem> TreeView<T> {
    ...
    pub fn reveal_item(&mut self, segments: Vec<String>) -> Result<()> {
        ...
        segments.iter().try_fold(&mut self.tree, |current_tree, segment| {
            match current_tree
                .children
                .iter_mut()
...
impl<T: TreeViewItem + Clone> TreeView<T> {
    pub fn render(
        ...
        for (index, line) in iter {
            ...
            surface.set_stringn(area.x, area.y, &line.indent, indent_len as usize, style);
...
mod test_tree_view {
    ...
    fn item(name: &str) -> DivisibleItem<'_> {
        DivisibleItem { name }
    }
...
⏵ krabby_patty
    ...
    mod static_tree {
        ...
        pub fn child(name: &str) -> StaticItem<'_> {
            StaticItem {
                name,
                children: None,
            }
