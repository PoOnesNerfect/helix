...
pub struct Config {
    ...
    /// Configuration for the file explorer (position and column width).
...
impl From<CloseError> for anyhow::Error {
    fn from(error: CloseError) -> Self {
        match error {
            ...
            CloseError::BufferModified(error) => anyhow::anyhow!("Buffer modified: '{error}'"),
            CloseError::SaveError(error) => error,
        }
    }
}
